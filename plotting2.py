"""Generate Monte Carlo summary plots for main2 experiments."""
...
def _collect_history_metrics(histories: List[dict]) -> Dict[str, List[np.ndarray]]:
    """Extract per-metric series from the history payload."""
...
def _collect_metric_row_metrics(metric_rows: List[List[dict]]) -> Dict[str, List[np.ndarray]]:
    """Extract numeric columns from metric_rows payload."""
    metrics: Dict[str, List[np.ndarray]] = {}
...
def _stack_runs(series_list: List[np.ndarray]) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Pad variable-length runs with NaNs and return mean/std envelopes."""
    if not series_list:
...
def main() -> None:
    ...
    results_dir = results_root / "main2"
    ...
    ckf_ref = payload.get("ckf_avg_residual", {})  # retained for optional baseline plotting
    ...
    for key, series in row_metrics.items():
        if key in all_metrics:
            continue  # avoid duplicating metrics already captured via history
        all_metrics[key] = series

    # Derived whole-matrix errors from per-block absolute errors
    ...
    for metric_name, series_list in sorted(all_metrics.items()):
        ...
        if metric_name.startswith("local_loss_"):
            ...
            # CKF baseline disabled temporarily; uncomment to display centralized reference.
    ...
    # Assemble averaged matrices and export to CSV
