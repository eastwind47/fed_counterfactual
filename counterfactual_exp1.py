...
"""The difference between this and counterfactual_exp.py is that it uses trained B_mn instead of B_true_offdiag for computing delta_phi"""
...
DELTA = 1  # step magnitude added to the intervened client input
...
SAVE_ARTIFACTS = False  # set True to save CSVs/plots for the first MC run
...
def main() -> None:
    ...
    def run_once(seed_override: int | None = None, save: bool = False, run_id: int = 1) -> dict:
        ...
        X_cf_fed, Y_cf_fed = rollout_counterfactual(A_mm, B_mm, A_mn_trained, B_mn_trained, C_mm, phi_trained, abducted_states, U_blocks_cf, delta_u_blocks, p_vec, s_vec, d_vec)
        ...
        delta_y_pct_err_norm = {cid: (delta_y_diff_norm[cid] / max(1e-9, abs(delta_y_base[cid]))) * 100.0 for cid in delta_y_diff_norm.keys()}
        rel_err_y_fed_t = {
            cid: float(
        ...
        percent_err_y_fed_t = {cid: rel_err_y_fed_t[cid] * 100.0 for cid in rel_err_y_fed_t.keys()}
        sym_rel_err_y_fed_t = {
            cid: float(
                np.linalg.norm(Y_cf_fed[cid][:, t_eval:t_eval + 1] - Y_blocks_true_cf[cid][:, t_eval:t_eval + 1])
                / max(
        ...
        norm_y_cf_t = {
            cid: float(np.linalg.norm(Y_cf_fed[cid][:, t_eval:t_eval + 1])) for cid in Y_cf_fed.keys()
        }
        norm_y_true_t = {
            cid: float(np.linalg.norm(Y_blocks_true_cf[cid][:, t_eval:t_eval + 1])) for cid in Y_blocks_true_cf.keys()
        }
        # Effect magnitude for fed vs true
        delta_fed = {
            cid: float(np.linalg.norm(Y_cf_fed[cid][:, t_eval:t_eval + 1] - y_pred_base[cid][:, t_eval:t_eval + 1]))
            for cid in Y_cf_fed.keys()
        }
        delta_true_effect = {
            cid: float(np.linalg.norm(Y_blocks_true_cf[cid][:, t_eval:t_eval + 1] - Y_blocks_base[cid][:, t_eval:t_eval + 1]))
            for cid in Y_blocks_true_cf.keys()
        }
        delta_effect_err = {cid: abs(delta_fed[cid] - delta_true_effect[cid]) for cid in delta_fed.keys()}
        delta_effect_err_pct = {
            cid: (delta_effect_err[cid] / max(1e-9, abs(delta_true_effect[cid]))) * 100.0 for cid in delta_effect_err.keys()
        }
        if save:
        ...
        return {
            ...
            "percent_err_y_fed_t": percent_err_y_fed_t,
            "sym_rel_err_y_fed_t": sym_rel_err_y_fed_t,
            "norm_y_cf_t": norm_y_cf_t,
            "norm_y_true_t": norm_y_true_t,
            "delta_fed": delta_fed,
            "delta_true_effect": delta_true_effect,
            "delta_effect_err": delta_effect_err,
            "delta_effect_err_pct": delta_effect_err_pct,
        }
    ...
    avg_percent_err_y_fed_t = average_dict("percent_err_y_fed_t")
    avg_sym_rel_err_y_fed_t = average_dict("sym_rel_err_y_fed_t")
    avg_norm_y_cf_t = average_dict("norm_y_cf_t")
    avg_norm_y_true_t = average_dict("norm_y_true_t")
    avg_delta_fed = average_dict("delta_fed")
    avg_delta_true_effect = average_dict("delta_true_effect")
    avg_delta_effect_err = average_dict("delta_effect_err")
    avg_delta_effect_err_pct = average_dict("delta_effect_err_pct")
    ...
    print("Avg Percent error (effect gap / baseline) at t+1 (%):", avg_delta_y_pct_err)
    print("Avg Percent error (effect gap per-dim / baseline) at t+1 (%):", avg_delta_y_pct_err_norm)
    print("Avg Relative error (CF vs true y) at t+1:", avg_rel_err_y_fed_t)
    print("Avg Percent error (CF vs true y) at t+1 (%):", avg_percent_err_y_fed_t)
    print("Avg Federated symmetric relative error y at t+1:", avg_sym_rel_err_y_fed_t)
    print("Avg ||y_cf_fed|| at t+1:", avg_norm_y_cf_t)
    print("Avg ||y_cf_true|| at t+1:", avg_norm_y_true_t)
    print("Avg Delta_fed (||y_cf_fed - y_base_fed||) at t+1:", avg_delta_fed)
    print("Avg Delta_true (||y_cf_true - y_base_true||) at t+1:", avg_delta_true_effect)
    print("Avg Abs(delta_fed - delta_true) at t+1:", avg_delta_effect_err)
    print("Avg Percent error (delta_fed vs delta_true) at t+1 (%):", avg_delta_effect_err_pct)
